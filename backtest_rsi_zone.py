#!/usr/bin/env python3
"""
BACKTEST: RSI Zone Strategy
Using Yahoo Finance Data

Strategy:
- RSI below 30 (oversold) = LONG signal (buy the dip)
- RSI above 70 (overbought) = SHORT signal (sell the rip)
- Use 5-minute candles for signals
...
print("\n" + "="*70)
print("RSI ZONE STRATEGY BACKTEST - YAHOO FINANCE")
print("="*70 + "\n")
...
COOLDOWN_MINUTES = 15  # Longer cooldown for RSI strategy

# RSI Settings
RSI_PERIOD = 14
RSI_OVERSOLD = 30   # Below this = LONG signal
RSI_OVERBOUGHT = 70  # Above this = SHORT signal
...
print(f"  Oversold (LONG): RSI < {RSI_OVERSOLD}")
print(f"  Overbought (SHORT): RSI > {RSI_OVERBOUGHT}")
print(f"  Exit Level: RSI crosses {RSI_EXIT_LEVEL}")
...
for current_date in all_dates:
    ...
    # Close any overnight positions (safety)
    ...
    for symbol, df in all_data.items():
        ...
        # Only trade during market hours (9:30 AM - 3:45 PM, leave buffer before close)
        ...
        for idx, candle in trading_data.iterrows():
            ...
            if symbol in open_positions:
                ...
                if pos['type'] == 'LONG' and rsi >= RSI_EXIT_LEVEL:
                    exit_price = close_price
                    exit_reason = 'RSI_EXIT'
                elif pos['type'] == 'SHORT' and rsi <= RSI_EXIT_LEVEL:
                    exit_price = close_price
                    exit_reason = 'RSI_EXIT'
            ...
            if rsi < RSI_OVERSOLD:
                signal_type = 'LONG'  # Oversold - expect bounce
                long_signals += 1
            elif rsi > RSI_OVERBOUGHT:
                signal_type = 'SHORT'  # Overbought - expect pullback
                short_signals += 1
...
print("\n" + "="*70)
print("BACKTEST RESULTS - RSI ZONE STRATEGY")
print("="*70)
...
print(f"\nLONG Trades:  {len(long_trades)} (signals: {long_signals})")
print(f"SHORT Trades: {len(short_trades)} (signals: {short_signals})")
...
rsi_exits = len([t for t in closed_trades if t.get('exit_reason') == 'RSI_EXIT'])
...
print(f"RSI Exit (neutral): {rsi_exits}")
...
if long_trades:
    ...
    print(f"\nLONG Performance:  ${long_pnl:+.2f} ({long_wr:.1f}% WR)")

if short_trades:
    short_pnl = sum(t['pnl'] for t in short_trades)
    short_wr = len([t for t in short_trades if t['pnl'] > 0]) / len(short_trades) * 100
    print(f"SHORT Performance: ${short_pnl:+.2f} ({short_wr:.1f}% WR)")
...
print(f"\nResults saved to backtest_rsi_results.json")
print("\n" + "="*70)
