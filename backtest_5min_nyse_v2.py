#!/usr/bin/env python3
"""
BACKTEST: 5-Minute NYSE Opening Range Breakout Strategy V2
- Fixed position sizing logic
- 3-minute cooldown after exits
- One trade per asset (no re-entry same day)
- JSON logging
"""
...
print("\n" + "="*70)
print("5-MINUTE NYSE OPENING RANGE BREAKOUT STRATEGY V2")
...
# Strategy
SL_BUFFER = 0.02
COOLDOWN_MINUTES = 3  # Cooldown period after exit
...
print(f"Stop Loss Buffer: {SL_BUFFER*100}%")
...
open_positions = {}
closed_trades = []
trade_log = []  # Detailed log for JSON
...
# Track cooldowns and daily trades per pair
pair_cooldowns = {}  # {pair: datetime when cooldown ends}
daily_traded_pairs = {}  # {date: set(pairs that traded today)}

csv_file = 'backtest_5min_nyse_v2_results.csv'
...
for date_idx, date_val in enumerate(all_dates):
    ...
    if previous_date != date_val:
        ...
        # Reset daily traded pairs for new day
        if date_val not in daily_traded_pairs:
            daily_traded_pairs[date_val] = set()
        ...
        for idx, candle in m5_candles[pair].iterrows():
            ...
            if exit_price:
                ...
                trade_record = {
                    ...
                    'exit_time': str(candle['datetime_az']),
                    'type': position['type'],
                    ...
                    'pnl': float(pnl),
                    'balance_after': float(account_balance),
                    'peak_balance': float(peak_balance),
                    'drawdown_pct': float(drawdown_pct),
                    'rsi': float(position.get('rsi', 0)),
                    'vol_ratio': float(position.get('vol_ratio', 0))
                }
                ...
                # Set cooldown period for this pair
                cooldown_end = candle['datetime_az'] + timedelta(minutes=COOLDOWN_MINUTES)
                pair_cooldowns[pair] = cooldown_end
    ...
    for pair in valid_pairs:
        ...
        # Check if already traded this pair today (one trade per asset per day)
        if pair in daily_traded_pairs.get(date_val, set()):
            continue
        ...
        for idx, m5_candle in candles_after_open.iterrows():
            ...
            if pair in pair_cooldowns:
                if m5_candle['datetime_az'] < pair_cooldowns[pair]:
            ...
            if signal_type:
                ...
                if signal_type == "LONG":
                    ...
                    if risk_per_unit <= 0:
                        continue
                    # Take profit: 2:1 reward/risk (6% gain target)
                    take_profit = entry_price + (2 * risk_per_unit)
                else:
                    entry_price = entry_price * (1 - SLIPPAGE_PCT)
                    # Stop loss: 3% above entry price
                    stop_loss = entry_price * (1 + SL_BUFFER)
                ...
                # Apply safety cap: max position value is 30% of balance
                ...
                # Mark this pair as traded today
                daily_traded_pairs[date_val].add(pair)
...
print(f"\n" + "="*70)
print("5-MINUTE NYSE OPENING RANGE BACKTEST RESULTS V2")
print("="*70 + "\n")
...
json_output = {
    'backtest_info': {
        ...
        'version': 'v2',
        'strategy': '5-Minute NYSE Opening Range Breakout',
        'nyse_open_time_az': f"{NYSE_OPEN_HOUR_AZ:02d}:{NYSE_OPEN_MINUTE_AZ:02d}",
    ...
    'results': {
        ...
        'win_rate_pct': (winning_trades / len(closed_trades) * 100) if closed_trades else 0,
        'avg_win': (sum(wins) / len(wins)) if wins else 0,
        'avg_loss': (sum(losses) / len(losses)) if losses else 0,
        'largest_win': max(wins) if wins else 0,
        'largest_loss': min(losses) if losses else 0,
        'profit_factor': (abs(sum(wins)) / abs(sum(losses))) if wins and losses else 0,
        'circuit_breaker_triggered': circuit_breaker_triggered
    },
    'trades': trade_log
}

json_file = 'backtest_5min_nyse_v2_results.json'
